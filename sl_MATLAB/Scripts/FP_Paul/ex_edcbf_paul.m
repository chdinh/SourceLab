%%
% Includes
% nichts geaendert, nur zum ausprobieren angelegt
...
for j = 1:2
    ...
    for i = 1: length(Dur)
        ...
        p_Dipole.Idx = [101 1014];

        %
        if j == 1
            t_Simulator.SourceActivation.addActivation([p_Dipole.Idx(1,1) p_Dipole.Idx(1,2)], s1);
        elseif j ==2
            t_Simulator.SourceActivation.addActivation([1957], s1, 'nn', [1 0 0]); %; 0 1 0]);
            t_Simulator.SourceActivation.addActivation([8151], s2, 'nn', [0 1 0]); %; 0 1 0]);
        %
        end;
