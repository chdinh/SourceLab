function f1h=show_electrode_signals(dinf, elecLabels, minf)
...
fs = 1000;             % sampling frequency for the computation of the time values
...
% create YTickLabel (electrode labels)
if ~isempty(elecLabels)
NCh=length(elecLabels);
YTickLabel=(flipud(elecLabels))';
else
    NCh = size(dinf,2);
    N_Comp=linspace(1,size(dinf,2),size(dinf,2));
    YTickLabel=cell(1,size(dinf,2));
    for i=1:size(dinf,2)
        YTickLabel{1,i}=['C',num2str(N_Comp(1,i))];
...
f1h = figure('Visible', 'off', 'Name', 'EEG Data Browser', ...
    'Tag', 'showElectrodeSignals',...
             'MenuBar', 'figure', 'Toolbar', 'none', ...
...
p1h = uipanel('Parent', f1h, 'Title', '', 'Position', [0 0 1 1]);

% axis to show the data
a1h = axes('Parent', p1h, 'Position', [0.08 0.06 0.87 0.89], ...
           ...
           'XAxisLocation', 'bottom', 'DrawMode', 'fast');
   
  % only display every second electrode label for EEG systems > 10-20
  if length(YTickLabel) > 21
  YTick=get(a1h,'YTick');
  YTickNew=YTick(1:2:end);
  YTickLabel=get(a1h,'YTickLabel');
  YTickLabelNew=YTickLabel(1:2:end);    
  set(a1h,'YTickLabel',YTickLabelNew);
  set(a1h,'YTick',YTickNew);
  end
  
 xLabel(a1h,'Time (ms)');
 if  ~isempty(elecLabels)
 yLabel(a1h,'Electrode label');
 else
     yLabel(a1h,'Components');
 end
...
mh4 = uimenu(a1menu, 'Label', 'Open in new window...', 'Callback', {@open_in_new_window});
...
s1h = uicontrol('Parent', p1h, 'Style', 'slider', 'Min', 1, 'Max', Nsamp - NWin_init, ...
                ...
                'Units', 'normalized', 'Position', [0.125 0.96 0.78 0.025], ...
...
b3h = uicontrol('Parent', p1h, 'Style', 'pushbutton', 'String', 'X-', ...
                'Units', 'normalized', 'Position', [0.077 0.96 0.025 0.025], ...
                'Callback', {@inc_x_scale});
            
b4h = uicontrol('Parent', p1h, 'Style', 'pushbutton', 'String', 'X+', ...
                'Units', 'normalized', 'Position', [0.929 0.96 0.025 0.025], ...
...
b5h = uicontrol('Parent', p1h, 'Style', 'pushbutton', 'String', '<', ...
                'Units', 'normalized', 'Position', [0.102 0.96 0.02 0.025], ...
                'Callback', {@dec_slider});
            
b6h = uicontrol('Parent', p1h, 'Style', 'pushbutton', 'String', '>', ...
                'Units', 'normalized', 'Position', [0.909 0.96 0.02 0.025], ...
                'Callback', {@inc_slider});
...
%% Support Functions
    ...
    function open_in_new_window(hObject,eventdata)
